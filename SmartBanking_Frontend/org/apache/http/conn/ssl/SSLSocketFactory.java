...
public class SSLSocketFactory implements SecureSocketFactory {
    ...
    public SSLSocketFactory(String algorithm, KeyStore keystore, String keystorePassword, KeyStore truststore, SecureRandom random) throws NoSuchAlgorithmException, KeyManagementException, KeyStoreException, UnrecoverableKeyException {
    ...
    public Socket connectSocket(Socket sock, String host, int port, InetAddress localAddress, int localPort, HttpParams params) throws IOException {
        if (host == null) {
            ...
            SSLSocket sslock = sock != null ? sock : createSocket();
            if (localAddress != null || localPort > 0) {
                if (localPort < 0) {
                    localPort = 0;
                }
                sslock.bind(new InetSocketAddress(localAddress, localPort));
            }
            sslock.connect(target, HttpConnectionParams.getConnectionTimeout(params));
            try {
                this.hostnameVerifier.verify(host, sslock);
                return sslock;
