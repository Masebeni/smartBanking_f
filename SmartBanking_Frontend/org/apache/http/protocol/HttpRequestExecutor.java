    ...
    protected HttpResponse doSendRequest(HttpRequest request, HttpClientConnection conn, HttpContext context) throws IOException, HttpException {
        if (request == null) {
            ...
            if (request instanceof HttpEntityEnclosingRequest) {
                ...
                if (((HttpEntityEnclosingRequest) request).expectContinue() && ver.greaterEquals(HttpVersion.HTTP_1_1)) {
                    conn.flush();
                    if (conn.isResponseAvailable(request.getParams().getIntParameter(HttpProtocolParams.WAIT_FOR_CONTINUE, 2000))) {
